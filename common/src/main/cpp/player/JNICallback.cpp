...
JNICallback::JNICallback(JavaVM *javaVM, JNIEnv *env, jobject instance) {
    ...
    this->jmd_repared = env->GetMethodID(mPlayClass, "onPrepared", sigPre);
    this->jmd_error = env->GetMethodID(mPlayClass, "onError", sigErr);
    this->jmid_progress = env->GetMethodID(mPlayClass, "onProgress", sigPro);
}
